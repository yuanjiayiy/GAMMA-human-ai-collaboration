...
vae_model_dir=${path}/pop_data/${pop}/vae_models/finetune_entire_kl_46
...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=0 python overcooked_population/eval_vae.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --layout_name ${layout} --num_agents ${num_agents} \
        --seed ${seed} --n_training_threads 1 --n_rollout_threads 120 --episode_length 400 --reward_shaping_horizon 0 \
        ...
        --vae_model_dir ${vae_model_dir} --dataset_file "../../../overcooked_dataset/${pop}/dataset.hdf5" --use_wandb
done
