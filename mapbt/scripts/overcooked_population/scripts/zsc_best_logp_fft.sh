...
if [ "$layout" == "counter_circuit_o_1order" ]; then
    base_kl=5
else
    base_kl=7
fi
...
num_agents=2
algo="adaptive"
exp="${0##*/}_${1}_kl-${kl_term}"


path=./overcooked_population
population_yaml_path=${path}/pop_data/${pop}/zsc_config.yml
vae_model_dir=${path}/pop_data/${pop}/vae_models/fft_kl_${base_kl}
...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=$2 python overcooked_population/train_coordinator_vs_vae.py \
